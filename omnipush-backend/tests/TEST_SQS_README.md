...
### 1. Test Debug Mode - Full Integration 🐛 (DEFAULT for 'single')

**'single' scenario now ALWAYS runs in debug mode** - invokes actual `process_newsit_message()`

```bash
# Install dependencies
uv sync

# Run debug mode with actual service (from project root)
...
### 2. View SQS Message Samples (No Setup)
```bash
python3 tests/test_sqs_message.py --samples-only
```

### 3. Test Other Scenarios (Mock Mode)
```bash
