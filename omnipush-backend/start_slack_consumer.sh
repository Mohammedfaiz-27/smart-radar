...
# Slack SQS Consumer Startup Script
# Manages the Slack SQS consumer background worker for Reshare Posts

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKER_SCRIPT="$SCRIPT_DIR/workers/slack_consumer.py"
PID_FILE="$SCRIPT_DIR/slack_consumer.pid"
LOG_FILE="$SCRIPT_DIR/logs/slack_consumer.log"
...
start_consumer() {
    print_info "Starting Slack SQS Consumer (Reshare Posts)..."
    ...
    if [ ! -f "$SCRIPT_DIR/.env" ]; then
        print_warning ".env file not found. Make sure AWS_SLACK_CONSUMER_SQS_QUEUE_URL is configured."
    fi
    ...
    sleep 2
    if is_running; then
        print_success "Slack SQS Consumer started successfully (PID: $PID)"
...
stop_consumer() {
    print_info "Stopping Slack SQS Consumer..."
...
restart_consumer() {
    print_info "Restarting Slack SQS Consumer..."
    stop_consumer
    sleep 2
    start_consumer
}
...
status_consumer() {
    if is_running; then
        PID=$(cat "$PID_FILE")
        print_success "Slack SQS Consumer is running (PID: $PID)"
        ...
        print_warning "Slack SQS Consumer is not running"
        return 1
    fi
}
...
show_usage() {
    cat << EOF
${BLUE}Slack SQS Consumer Management Script (Reshare Posts)${NC}

Usage: $0 {start|stop|restart|status|logs}

Commands:
  start     Start the Slack SQS consumer
  stop      Stop the Slack SQS consumer
  restart   Restart the Slack SQS consumer
...
Environment:
  Configure AWS_SLACK_CONSUMER_SQS_QUEUE_URL in .env file
